package com.hl.policeweather.listener;
...
public class GyroscopeObserver implements SensorEventListener
{
    ...
    // The maximum radian that the device should rotate along x-axis and y-axis
    // to show image's bounds
    ...
    public void onSensorChanged(SensorEvent event)
    {
        ...
        if (rotateY > rotateX + rotateZ)
        {
            ...
            {
                mView.setSensorChangedFraction(
                        (float) (mRotateRadianY / mMaxRotateRadian));
            }
        }
        mLastTimestamp = event.timestamp;
    }
    ...
    public void setMaxRotateRadian(double maxRotateRadian)
    {
        if (maxRotateRadian <= 0 || maxRotateRadian > Math.PI / 2)
        {
            throw new IllegalArgumentException(
                    "The maxRotateRadian must be between (0, π/2].");
        }
